...
set variable_field=2, version=version+1
...
```mysql
alter table tb_stock add column `version` int default '0' comment '版本号' after count; 
```

SQL代码：
```mysql
update tb_stock set count=2, version = version + 1 where id='1' and version='0';
```
